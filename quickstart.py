...
class ChatFunctions:
    def __init__(self):
        ...
        self.limit1 = datetime.strptime("00:00:00", "%H:%M:%S").time()  # to avoid (-) times
        self.limit2 = datetime.strptime("23:59:59", "%H:%M:%S").time()  # highest you can go
    ...
    def chat_completion_request(self, messages, functions=None, function_call=None, model=None, api_key=None):
    ...
    def deleting_events(self, arguments, service):
        try:
            provided_date = str(datetime.strptime(json.loads(arguments)['date'], "%Y-%m-%d").date())
            provided_time = str(
                datetime.strptime(json.loads(arguments)['time'].replace("PM", "").replace("AM", "").strip(),
    ...
    def check_availability(self, arguments, service):
        try:
            ...
            if start_date_time < datetime.now(timezone):
                ...
                if start_date_time.time() >= self.limit1 and start_date_time.time() <= self.limit2:
